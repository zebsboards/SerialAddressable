...
namespace DirectOutput.Cab.Out.AdressableLedStrip
{
    ...
    public class ZBSerial14Port : OutputControllerCompleteBase
    {
        // array to hold the number of leds per strip
		
		protected int[] NumberOfLedsPerStrip = new int[14];
		
		private int _NumberOfStrips = 8;

        /// Gets or sets the number of strips being used (default of 8 strips, max 14 strips) 
        
        public int NumberOfStrips
        {
            get { return _NumberOfStrips; }
            set { _NumberOfStrips = value; }
        }
        
        /// Gets or sets the number of leds of ledstrip connected to channel 1 - 14
        ...
        private int _ComPortBaudRate = 2000000;
        ...
        private bool _ComPortDtrEnable = true;
        ...
        private bool _ComPortRtsEnable = false;

        /// <summary></summary>
        /// Gets or sets the COM port RTS (READY TO SEND) enable state.
        public bool ComPortRtsEnable
        {
            get { return _ComPortRtsEnable; }
            set { _ComPortRtsEnable = value; }
        }
        ...
        {
            var NrOfLeds = OutputValues.Length / 3; // reduce number of leds to number of pixels (1 pixel = R+G+B)
        ...
        protected override void UpdateOutputs(byte[] OutputValues)
        {
            ...
            for (int i = 0; i < NumberOfStrips; i++)  // loop for number of strips
            {
                int NrOfLedsOnStrip = NumberOfLedsPerStrip[i];
                if (NrOfLedsOnStrip > 0)  // if ledstrip has leds assigned (strip is used)
                {
                    int TargetPosition = i * NumberOfLedsPerChannel; // defines strip being set by multiplying strip (loop position)


                    SendLedstripData(OutputValues.Skip(SourcePosition * 3).Take(NrOfLedsOnStrip * 3).ToArray(), TargetPosition);
                    //             position in outputValues array to use data from (every 4th byte starts the new triplet for the pixel), number of leds assigned to the strip (3 x pixel count), starting number of pixels on strip denotes which strip to use
                    ...
                    {
                        throw new Exception($"An exception occured while waiting for the ACK after sending the data for channel {i + 1} of the {this.GetType().ToString()}.", E);
                    }
                    if (BytesRead != 1 || AnswerData[0] != (byte)'A')
                    {
                        throw new Exception($"Received no answer or an unexpected answer while waiting for the ACK after sending the data for channel {i + 1} of the {this.GetType().ToString()}.");
                    }
            ...
            {
                throw new Exception($"An exception occured while waiting for the ACK after sending the output command (O) to the {this.GetType().ToString()}", E);
            }
            if (BytesRead != 1 || AnswerData[0] != (byte)'A')
            {
                throw new Exception($"Received no answer or an unexpected answer ({AnswerData[0]}) while waiting for the ACK after sending the output command (O) to the {this.GetType().ToString()}");
            }
        ...
        protected virtual void SetupController()
        {
            ...
            try
            {
                ...
                Log.Write($"Attempting communication with ZBSerial controller . . . . ");
                Log.Write($"Requested max number of leds per channel for ZBSerial controller, Received {ReceiveData[0]}, {ReceiveData[1]}, {ReceiveData[2]} (should be 4, 0, 65)");
            }
            catch (Exception E)
            {
            ...
            if (BytesRead != 3)
            {
                throw new Exception($"The {this.GetType().ToString()} did not receive the expected 3 bytes containing the data on the max number of leds per channel. Received only {BytesRead} bytes. Will not send data to the controller");
            }
            if (ReceiveData[2] == '0')
            {
                throw new Exception($"The {this.GetType().ToString()} sent {ReceiveData[2]} instead of an ACK after sending the max number of leds per channel. Will not send data to the controller");
            }
			...
			//Send number of strips in use to controller
            ushort NrOfStrips = (ushort)NumberOfStrips;
            byte[] CommandData = new byte[3] { (byte)'B', (byte)(NrOfStrips >> 8), (byte)(NrOfStrips & 255) };
            ...
            {
                throw new Exception("Expected 1 bytes after sending the number of channels being used, but the read operation resulted in a exception. Will not send data to the controller.", E);
            }

            if (BytesRead != 1 || ReceiveData[0] != (byte)'A')
            {
                throw new Exception("Expected an Ack (A) after nd the number of channels being used, but received no answer or a unexpected answer. Will not send data to the controller.");
            ...
            CommandData = new byte[3] { (byte)'L', (byte)(NrOfLeds >> 8), (byte)(NrOfLeds & 255) };
            ComPort.Write(CommandData, 0, 3);
            ReceiveData = new byte[1];
            BytesRead = -1;
            try
            ...
            if (BytesRead != 1 || ReceiveData[0] != (byte)'A')
            {
                throw new Exception("Expected an Ack (A) after setting the number of leds per channel, but received no answer or a unexpected answer. Will not send data to the controller.");
        ...
        protected override void ConnectToController()
        {
            ...
            ComPort.RtsEnable = ComPortRtsEnable;
            ...
            {
                throw new Exception($"An exception occured while setting the name of the Com-port '{ComPortName}'. Found the following Com-Ports: {string.Join(", ", PortNames)}.  Will not send data to the controller.", E);
            }
            ...
            {
                throw new Exception($"An exception occured while trying to open the Com-port '{ComPortName}'. Change the port assignment in the cabinet file to one of the following Com-Ports: {string.Join(", ", PortNames)}.  Will not send data to the controller.", E);
            }
            ...
            for (int AttemptNr = 0; AttemptNr < 20; AttemptNr++)
            {
                ...
                //int Ret = ReadPortWait(new byte[1], 0, 1);
            ...
            if (BytesRead != 1 || ReceiveData[0] != (byte)'A')
            {
                throw new Exception($"Expected an Ack (A) after clearing the buffer of the {this.GetType().ToString()}, but received no answer or a unexpected answer. Will not send data to the controller.");
            }
            ...
            {
                throw new Exception($"Expected 1 byte after attempting to run the output of the {this.GetType().ToString()} to the ledstrips, but the read operation resulted in a exception. Will not send data to the controller.", E);
            }

            if (BytesRead != 1 || ReceiveData[0] != (byte)'A')
            {
                throw new Exception($"Expected an Ack (A) after outputing the buffer of the {this.GetType().ToString()} to the ledstrips, but received no answer or a unexpected answer. Will not send data to the controller.");
            }
            else
            {
                Log.Write($"********** Setup of {NumberOfStrips} channels on the ZBSerial14 Controller Successfully Completed ********** Attempting to start Serial Updater Thread *********");
            }
        }
        ...
        protected int ReadPortWait(byte[] Buffer, int BufferOffset, int NumberOfBytes)
        {
            ...
            for (int ByteNumber = 0; ByteNumber < NumberOfBytes; ByteNumber++)
            {
                ...
                {
                    throw new Exception($"An exception occured while trying to read byte {ByteNumber + 1} of {NumberOfBytes} from Com-Port {ComPort.PortName}.", E);
                }

                if (BytesRead != 1)
                {
                    throw new Exception($"An exception occured while trying to read byte {ByteNumber + 1} of {NumberOfBytes} from Com-Port {ComPort.PortName}. Tried to read 1 byte, but received {BytesRead} bytes.");
                }
